...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  } else if (transaction.coinstake) {
    transformed.vin = [
      {
        coinstake: transaction.inputs[0].script,
        sequence: transaction.inputs[0].sequence,
        n: 0
      }
    ];
