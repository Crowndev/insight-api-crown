...
BlockController.prototype.getPoolInfo = function(block) {
  var coinbaseBuffer =
    (block.header.auxPow !== undefined ? block.header.auxPow : block.transactions[0])
    .inputs[0]._scriptBuffer;
  var str = coinbaseBuffer.toString('utf-8');
  ...
  var match = str.match(/\/([ A-Za-z0-9~`!@#$%^&*()_+={}\[\]|\\:;"'<>,.?-]{1,30})\//);
  if (match !== null) {
    str = match[1];
    str = str.match(/^(www\.)?(.+)$/)[2];
    if (str.length <= 25) {
...
BlockController.prototype.getBlockReward = function(height) {
  height--; // This algorithm applies to the previous height.
